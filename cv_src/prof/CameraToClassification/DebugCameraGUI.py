...
def launch_camera():
    ...
    camera.set(cv2.CAP_PROP_FRAME_WIDTH, 1920/6)
    camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 1080/6)
...
def save_camera_image(frame, file_counter, session_id):

    label = file_counter_to_label(file_counter)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    cv2.imwrite("CameraToClassification\Images\Saved_Images/" + label + "_" + session_id + "_" + str(file_counter) + ".jpg", frame)
...
# Apply nonlinear brightness adjustment
def apply_nonlinear_brightness(gray, threshold=128, dark_power=1.8, bright_power=0.6):
...
def apply_nonlinear_brightness_clip(gray, threshold=128, dark_power=1.8, bright_power=0.6, clip_threshold=0):
    a = gray.astype(np.float32) / 255.0
    t = threshold / 255.0
    out = np.empty_like(a)
    mask = a < t
...
if __name__ == "__main__":
    ...
    while running:
        ...
        if keys[pygame.K_KP5]:
            dark_power = min(60.0, dark_power + 0.5)
        if keys[pygame.K_KP2]:
            dark_power = max(0.01, dark_power - 0.1)
        # Adjust bright power
        ...
        red_gray_nl = apply_nonlinear_brightness_clip(red_gray, threshold, dark_power, bright_power, clip_threshold)
        red_gray_rgb_nl = cv2.cvtColor(red_gray_nl, cv2.COLOR_GRAY2RGB)
        red_gray_surface_nl = frame_to_surface(red_gray_rgb_nl)
        screen.blit(red_gray_surface_nl, index_to_position(1, 2, camera_frame.shape[1], camera_frame.shape[0]))
        ...
        screen.blit(masked_frame_surface, index_to_position(0, 3, camera_frame.shape[1], camera_frame.shape[0]))

        if save_image:
            # save_camera_image(red_gray_rgb_nl, file_counter, session_id)
            # Take camera_frame and red_gray_rgb_nl and use red_gray_rgb_nl as a mask to darken non-red areas
            save_camera_image(masked_frame, file_counter, session_id)
            file_counter += 1
